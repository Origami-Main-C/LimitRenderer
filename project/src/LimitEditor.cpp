...
int main() {
    ...
    glRenderbufferStorage(GL_RENDERBUFFER, GL_DEPTH24_STENCIL8, 2200,
                          1400); // use a single renderbuffer object for both a depth AND stencil buffer.
    ...
    while (!LimitEditor.ShouldClose()) {
        ...
        for (int i = 0; i < modelList.size(); ++i) {
            ...
            model = glm::translate(model,
                                   glm::vec3(modelsData[i].translate.x, modelsData[i].translate.y,
                                             modelsData[i].translate.z -
                                             1.0f)); // translate it down so it's at the center of the scene
            model = glm::scale(model, glm::vec3(0.1f * modelsData[i].scale.x, 0.1f * modelsData[i].scale.y,
                                                0.1f * modelsData[i].scale.z));
            model = glm::rotate(model, glm::radians(modelsData[i].rotate.x), glm::vec3(1.0f, 0.0f, 0.0f));
            model = glm::rotate(model, glm::radians(modelsData[i].rotate.y), glm::vec3(0.0f, 1.0f, 0.0f));
            model = glm::rotate(model, glm::radians(modelsData[i].rotate.z), glm::vec3(0.0f, 0.0f, 1.0f));
        ...
        if (!modelsData.empty()) {
            ...
            ImGui::SameLine();
            if (ImGui::ArrowButton("##right", ImGuiDir_Right) && cur_object < modelsData.size() - 1)
                cur_object++;
